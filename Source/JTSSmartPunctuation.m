...
typedef NS_ENUM(NSInteger, JTSQuoteDirection) {
    JTSQuoteDirection_Left,
    JTSQuoteDirection_Right,
};

// - - -

@interface NSString (JTSEmoji)

- (NSUInteger)jts_lengthAdjustingForComposedCharacters;

@end

@implementation NSString (JTSEmoji)
...
@interface JTSSmartPunctuationComposedCharacterItem : NSObject
...
@implementation JTSSmartPunctuation
...
+ (BOOL)_shouldAttemptCorrections:(NSTextStorage *)textStorage
    ...
    if (editedRange.length < 1) {
        ...
        NSInteger composedLength = [editedPortion jts_lengthAdjustingForComposedCharacters];
        if (composedLength != 1) {
    ...
    if (desiredPreviousCount) {
        [textStorage.string
         ...
         usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
             ...
             JTSSmartPunctuationComposedCharacterItem *item = [[JTSSmartPunctuationComposedCharacterItem alloc] init];
...
+ (BOOL)_attemptElipsisFix:(NSTextStorage *)textStorage
    ...
    if (count >= 3) {
        ...
        JTSSmartPunctuationComposedCharacterItem *typedItem = [targetCharacterItems lastObject];
        NSString *typedCharacter = typedItem.string;
        
        if ([typedCharacter isEqualToString:dot]) {
            JTSSmartPunctuationComposedCharacterItem *prevPrevItem = nil;
            JTSSmartPunctuationComposedCharacterItem *prevItem = nil;
...
+ (void)_attemptSmartQuoteFixes:(NSTextStorage *)textStorage
    ...
    for (JTSSmartPunctuationComposedCharacterItem *editableItem in editableCharacterItems) {
...
+ (NSString *)_replacementStringForSingleQuoteAtIndex:(NSInteger)index
    ...
    if (index > 0) {
        JTSSmartPunctuationComposedCharacterItem *prevItem = items[index-1];
        NSString *prevCharacter = prevItem.string;
        JTSQuoteDirection quoteDirection = [self _quoteDirectionThatShouldFollowCharacter:prevCharacter
                                                                             forDirection:writingDirection];
        if (quoteDirection == JTSQuoteDirection_Left) {
        ...
        if ((writingDirection == UITextWritingDirectionLeftToRight || writingDirection == UITextWritingDirectionNatural)
            && quoteDirection == JTSQuoteDirection_Left) {
            ...
            if ( (items.count - index - 1) >= 3) {
                ...
                JTSSmartPunctuationComposedCharacterItem *tensItem = items[tensPlaceIndex];
                JTSSmartPunctuationComposedCharacterItem *zerosItem = items[zerosPlaceIndex];
                JTSSmartPunctuationComposedCharacterItem *pluralItem = items[pluralSIndex];
...
+ (JTSQuoteDirection)_quoteDirectionThatShouldFollowCharacter:(NSString *)character
                                                 forDirection:(UITextWritingDirection)writingDirection {
    
    JTSQuoteDirection quoteDirection;
    ...
    if (writingDirection == UITextWritingDirectionLeftToRight || writingDirection == UITextWritingDirectionNatural) {
        ...
        if (regexRange.location != NSNotFound) {
            ...
            quoteDirection = JTSQuoteDirection_Right;
        }
    }
    else /* Right to Left */{
